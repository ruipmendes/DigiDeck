...
const VALID_ACTION_TYPES = new Set(['hotkey', 'text', 'launch', 'url', 'script', 'volume', 'sound', 'mic', 'app-audio', 'obs', 'streamlabs', 'twitch', 'twitch-streamer', 'kick', 'kick-streamer', 'discord', 'spotify', 'hue', 'homeassistant', 'openrgb', 'nanoleaf', 'mixitup', 'voicemeeter', 'voicemod', 'goto-page', 'wait']);
...
function validateButtons(input: unknown[], seenIds: Set<number>): Tile[] {
  ...
  for (const t of input) {
    ...
    if (kind === 'slider') {
      ...
      if (tile.provider !== undefined && tile.provider !== 'obs' && tile.provider !== 'streamlabs' && tile.provider !== 'discord' && tile.provider !== 'spotify' && tile.provider !== 'app-audio' && tile.provider !== 'hue' && tile.provider !== 'homeassistant' && tile.provider !== 'nanoleaf' && tile.provider !== 'voicemeeter') {
        throw new Error(`tile ${tile.id}: slider provider must be 'obs', 'streamlabs', 'discord', 'spotify', 'app-audio', 'hue', 'homeassistant', 'nanoleaf', or 'voicemeeter'`);
      }
