...
export function ObsPanel() {
  ...
  async function toggleEnabled() {
    ...
    setError(null);
    try {
      const data = await api.putObsConfig({ ...config, enabled: !config.enabled });
      setConfig(data.config);
      setStatus(data.status);
    } catch (e) {
      setError((e as Error).message);
    ...
    <div style={{ background: '#111827', border: '1px solid #1f2937', borderRadius: 10, padding: 14 }}>
      ...
      {expanded && config && (
        <div style={{ marginTop: 14, marginLeft: 26, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {state === 'disabled' ? (
            <button onClick={toggleEnabled} disabled={busy} style={primaryBtn}>Enable OBS integration</button>
          ) : (
            <>
              <div style={grid}>
