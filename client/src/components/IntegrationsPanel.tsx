...
type Summary = {
  ...
  nanoleaf:   { enabled: boolean; state: StatusKind };
};
...
const DEFAULT_SUMMARY: Summary = {
  ...
  nanoleaf:   { enabled: false, state: 'disabled' },
};
...
export function IntegrationsPanel() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [obs, sl, tw, kk, dc, sp, hu, ha, org, nl] = await Promise.all([
    ...
    <div style={{ background: '#0a0a0a', border: '1px solid #1f2937', borderRadius: 10 }}>
      ...
      >
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 12, marginLeft: 12, flexWrap: 'wrap' }}>
          ...
          <Pill name="Nanoleaf"   s={summary.nanoleaf} />
        </span>
      </button>
